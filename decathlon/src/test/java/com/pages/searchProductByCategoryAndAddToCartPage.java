...
public class searchProductByCategoryAndAddToCartPage 
{
	...
	{
		if(!driver.getTitle().equals("https://www.decathlon.in"))
			...
			{				
				for(int column = 1; column < totalColumns; column++)
				{
					...
					else if(excel.readExcelData("Sheet2",0,column).equalsIgnoreCase("PIN CODE"))
					{
						...
						excel.writeExcelData("Sheet2",row, (column+2), ProductID,"blue");
						
						//To write STATUS back to excel
						if(excel.readExcelData("Sheet2", row, 5).equalsIgnoreCase(excel.readExcelData("Sheet2", row, 6)))
								excel.writeExcelData("Sheet2",row, 9, "PASS","green");
						else
							excel.writeExcelData("Sheet2",row, 9, "PASS","red");
					}
				}
			}
		}
